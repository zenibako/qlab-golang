...
func PerformScopeBasedComparison(sourceCueData, cachedCueData, currentQLabData map[string]any) (*ScopeComparison, error) {
	...
	sourceCues := IndexCuesFromWorkspace(sourceCueData)
	cachedCues := IndexCuesFromWorkspace(cachedCueData)
	currentCues := IndexCuesFromWorkspace(currentQLabData)
	...
	allCueNumbers := getAllCueIdentifiers(sourceCues, cachedCues, currentCues)
	...
	for _, cueNumber := range allCueNumbers {
		...
		cueScope := compareCueScope(cueNumber, sourceCue, cachedCue, currentCue, hasSource, hasCache, hasCurrent)
...
func compareCueScope(cueNumber string, sourceCue, cachedCue, currentCue map[string]any, hasSource, hasCache, hasCurrent bool) *ScopeComparison {
	...
	allFields := getAllFieldNames(sourceCue, cachedCue, currentCue)

	hasChanges := false
	hasConflicts := false

	for _, fieldName := range allFields {
		fieldConflict := compareField(fieldName, sourceCue, cachedCue, currentCue, hasCache)
		...
		if fieldConflict != nil {
			...
			if isFieldConflict(fieldConflict) {
				hasConflicts = true
				hasChanges = true
			} else if !reflect.DeepEqual(fieldConflict.SourceValue, fieldConflict.QLabValue) {
				hasChanges = true
			}
		}
	}
...
func compareField(fieldName string, sourceCue, cachedCue, currentCue map[string]any, hasCache bool) *FieldConflict {
	sourceVal := sourceCue[fieldName]
	cacheVal := cachedCue[fieldName]
	currentVal := currentCue[fieldName]

	// Normalize values for comparison
	sourceNorm := normalizeProperty(sourceVal)
	cacheNorm := normalizeProperty(cacheVal)
	currentNorm := normalizeProperty(currentVal)

	// Use smart comparison for specific field types
	sourceMatchesCurrent := comparePropertyValues(fieldName, sourceNorm, currentNorm)
	...
	sourceMatchesCache := comparePropertyValues(fieldName, sourceNorm, cacheNorm)
	cacheMatchesCurrent := comparePropertyValues(fieldName, cacheNorm, currentNorm)
...
func isFieldConflict(fc *FieldConflict) bool {
	if fc == nil {
		return false
	}
	...
	sourceMatchesCache := comparePropertyValues(fc.FieldName, sourceNorm, cacheNorm)
	cacheMatchesQLab := comparePropertyValues(fc.FieldName, cacheNorm, qlabNorm)
	...
	if !sourceMatchesCache && !cacheMatchesQLab {
		sourceMatchesQLab := comparePropertyValues(fc.FieldName, sourceNorm, qlabNorm)
...
func getAllCueIdentifiers(sourceCues, cachedCues, currentCues map[string]map[string]any) []string {
...
func getAllFieldNames(sourceCue, cachedCue, currentCue map[string]any) []string {
...
func GenerateMergedScope(scopeComparison *ScopeComparison, comparison *ThreeWayComparison) (*MergedScope, error) {
	...
	for _, childScope := range scopeComparison.ChildScopes {
		childMerged, err := GenerateMergedScope(childScope, comparison)
		if err != nil {
...
func ExtractMergedWorkspaceData(mergedScope *MergedScope) (map[string]any, error) {
	if mergedScope == nil {
